...
class WXLOG_Custom_reply_edit {
	...
	public function edit_page() {
		...
		<div class="wrap">
            ...
            <tr valign="top" class="set-news"<?php if($custom_reply->msgtype!='news') echo ' style="display:none;"';?>><th scope="row">图文内容</th>
			...
			foreach($wxlog_news_list_title as $key=>$value){?>
                <table class="form-table wxlog_news_list" id="wxlog_news_<?=($key+1)?>" style="padding-top:15px;">
                    ...
                    <input name="image_url[]" type="text" id="image_url<?=($key+1)?>" value="<?=$wxlog_news_list_image_url[$key]?>" class="regular-text"><input onclick="del_wxlog_upload('<?=($key+1)?>');" type="button" value=" 上传 " class="button dp-upload-button">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a href="javascript:del_wxlog_news('wxlog_news_<?=($key+1)?>');">删除图文</a>
                    </td>
                    </tr>
                </table>
            <?php }?>
            ...
            <tr valign="top" class="set-text"><th scope="row">回复内容</th><td>
